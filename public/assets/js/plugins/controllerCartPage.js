...
function renderListProduct(){
    ...
    for(let i=0; i< products.length; i++){
        ...
        if (products[i].Quantity > 1) {
            str1 += '<div class="dec qtybutton" onclick="changeQuantity(\'' + products[i].cart_id + '\',\'down\')">'
                + '<button class="fa fa-angle-down"></button></div>';
            htmlCart += str1;
        } else {
            str1 += '<div class="dec qtybutton">'
                + '<button disabled class="fa fa-angle-down"></button></div>'
            htmlCart += str1;
        }
        htmlCart += '<div class="inc qtybutton" onclick="changeQuantity(\'' + products[i].cart_id + '\',\'up\')"><button class="fa fa-angle-up"></button></div>'
            + '</div>'
            + '</td>'
            + '<td class="product-subtotal">'
            + '<span id="total' + i + '" class="amount">$' + total.toFixed(2) + '</span>'
            + '</td>'
            + '</tr>';
    }
